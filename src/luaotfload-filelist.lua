...
As the order how files are loaded during the merge matters the table is an array.
Lists with other sorting should be created with functions.
...
- context     prefix: ctx
- git source  prefix: git
- texmf       prefix: tex

The "flow" is always from context->git->texmf, so the "source" of a file can be deduced by the dirs recorded in the entries 
