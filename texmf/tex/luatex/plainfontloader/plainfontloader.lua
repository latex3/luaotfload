if not modules then modules = { } end modules ['plainfontloader'] = {
    version   = 1.001,
    comment   = "a adapted copy of luatex-fonts.lua, to test the fontloader outside context",
    author    = "Ulrike Fischer/ Hans Hagen, PRAGMA-ADE, Hasselt NL",
...
--
--   mtxrun --script context luatex-basics-prepare.tex % creates luatex-basics-chr.lua, correctly copied manually to plainfontloader-basic-chr.lua
--
...
if non_generic_context.luatex_fonts.skip_loading ~= true then
    ...
    loadmodule('fontloader-reference.lua',true)
    ...
    if fonts then
        ...
        loadmodule("fontloader-l-lua.lua")
        loadmodule("fontloader-l-lpeg.lua")
        loadmodule("fontloader-l-function.lua")
        loadmodule("fontloader-l-string.lua")
        loadmodule("fontloader-l-table.lua")
        loadmodule("fontloader-l-io.lua")
        loadmodule("fontloader-l-file.lua")
        loadmodule("fontloader-l-boolean.lua")
        loadmodule("fontloader-l-math.lua")
        loadmodule("fontloader-l-unicode.lua")

        -- A few slightly higher level support modules:

        loadmodule("fontloader-util-str.lua")
        loadmodule("fontloader-util-fil.lua")
        ...
        loadmodule('fontloader-basics-nod.lua')

        -- We ship a resources needed for font handling (more might end up here).

        loadmodule('fontloader-basics-chr.lua')
        ...
        loadmodule('fontloader-font-ini.lua')
        loadmodule('fontloader-fonts-mis.lua')
        loadmodule('fontloader-font-con.lua')
        loadmodule('fontloader-fonts-enc.lua')
        loadmodule('fontloader-font-cid.lua')
        loadmodule('fontloader-font-map.lua')
        ...
        loadmodule('fontloader-fonts-syn.lua')

        -- We need some helpers.

        loadmodule('fontloader-font-vfc.lua')

        -- This is the bulk of opentype code.

        loadmodule('fontloader-font-otr.lua')
        loadmodule('fontloader-font-oti.lua')
        loadmodule('fontloader-font-ott.lua')
        loadmodule('fontloader-font-cff.lua')
        loadmodule('fontloader-font-ttf.lua')
        loadmodule('fontloader-font-dsp.lua')
        loadmodule('fontloader-font-oup.lua')
        loadmodule('fontloader-font-otl.lua')
        loadmodule('fontloader-font-oto.lua')
        loadmodule('fontloader-font-otj.lua')
        loadmodule('fontloader-font-ota.lua')
        loadmodule('fontloader-font-ots.lua')
        loadmodule('fontloader-font-osd.lua')
        loadmodule('fontloader-font-ocl.lua')
        loadmodule('fontloader-font-otc.lua')

        -- The code for type one fonts.

        loadmodule('fontloader-font-onr.lua')
        loadmodule('fontloader-font-one.lua')
        loadmodule('fontloader-font-afk.lua')
        ...
        loadmodule('fontloader-font-lua.lua')
        loadmodule('fontloader-font-def.lua')

        -- We support xetex compatible specifiers (plain/latex only).

        loadmodule('fontloader-fonts-def.lua') -- was font-xtx.lua

        -- Here come some additional features.

        loadmodule('fontloader-fonts-ext.lua')
        loadmodule('fontloader-font-imp-tex.lua')
        loadmodule('fontloader-font-imp-ligatures.lua')
        loadmodule('fontloader-font-imp-italics.lua')
        loadmodule('fontloader-font-imp-effects.lua')
        loadmodule('fontloader-fonts-lig.lua')

        -- We need to plug into a callback and the following module implements the
        -- handlers. Actual plugging in happens later.

        loadmodule('fontloader-fonts-gbn.lua')
...
logs.report("system","plainfontloader.lua loaded in %0.3f seconds", os.gettimeofday()-starttime)
